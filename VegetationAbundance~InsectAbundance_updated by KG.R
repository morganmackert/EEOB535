...
#Calculate total insect abundance at each site on each date
...
#Poisson model for insect abundance predicted by vegetation cover with date as fixed effect
VAonIAmodel <- glm(Abundance ~ AverageVegCover + Date,
                   data = VAonIA,
                   family = poisson(link = "log"))
summary(VAonIAmodel)

#Graph insect abundance by average vegetation coverage
VAonIAplot <- ggplot(VAonIA, aes(x = AverageVegCover,
  ...
  ggtitle("Influence of Vegetation Cover on \nArthropod Abundance") +
